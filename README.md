...
2. **开启方式**——在 `~/.claude/skills/vault-loader/config.json` 加：

   ```json
   { "metrics": { "enabled": true } }
   ```

3. **记录什么**——prompt 关键词的**加盐 hash**（非明文；盐存在同目录下的 `.salt`，本机静态、启动时自动生成——**该文件本身须保密**，本机攻击者一旦读到它，就能对有限的关键词/路径空间做字典攻击、逆推出 hash 对应的明文）、被召回笔记的**命中词明文**、笔记相对路径、打分与闸门决策。**不记录 prompt 原文**。
4. **存在哪 + 体积**——`~/.claude/vault-loader-metrics/`。事件按月分桶落在 `<YYYY-MM>/` 子目录下，`metrics.retention_days`（默认 90 天）**只清理这部分**、在你正常使用时按「每天至多一次」自动触发，无需手动操作；同目录下的 `.salt`、`near_miss_counts.json`、`nudge_ts.json`、`prune_ts.json`、以及 `--review` 产生的 `annotations.jsonl`（人工标注）都**不**受 `retention_days` 约束，会一直留到你手动 `--purge`。

   单条事件记录体积实测约 **3.7～5.9 KB**（真实 Vault 多次采样）：命中数低于 `admitted_k` 上限（默认 20）时体积随命中数走，采样到 3.7~4.7 KB；命中数达到或超过上限后落盘按 `total` 降序截断到 20 条展示样本，体积趋于稳定、不再随查询范围继续增长（本次采样窄/中/宽三种查询宽度下均落在 5.4~5.9 KB，尽管真实候选数分别是 41/148/231 条）。总体积取决于你的实际提问频率，粗算公式：`单条约 4~6 KB × 提问次数`——举例，若日均提问 30 次，90 天累计约 2700 次，体积约 11～16 MB（区间宽是因为单条体积本身仍会波动，不是单一精确值；这不是一个已验证的日均基准，只是换算示例）。
5. **怎么清 + 残留告知**——报表与清空：

   ```bash
   VL=$(ls -d ~/.claude/plugins/cache/*/claude-vault/*/skills/vault-loader/scripts 2>/dev/null | sort -V | tail -1)
   python3 "$VL/analyze_metrics.py" --report   # 报表：命中分布、near-miss 候选（默认隐去路径，--show-paths 展开）
   python3 "$VL/analyze_metrics.py" --purge    # 单条命令立即不可逆删除，删除后打印删了多少个数据文件
   ```
