...
3. **记录什么**——prompt 关键词的**加盐 hash**（非明文；盐存在同目录下的 `.salt`，本机静态、启动时自动生成）、被召回笔记的**命中词明文**、笔记相对路径、打分与闸门决策。**不记录 prompt 原文**。
   ...
   > ⚠️ **`.salt` 的保护是平台相关的，Windows 上没有强制手段。** 加盐 hash 的安全性前提是这个文件保密——本机攻击者一旦读到它，就能对有限的关键词空间做字典攻击、逆推出 hash 对应的明文。POSIX 上由文件权限 `0600` 保证；**Windows 上 `chmod` 不生效**（NTFS 走 ACL，Python 的 `os.chmod` 对它基本无效），该文件的可读范围完全取决于 `~/.claude` 目录继承下来的 ACL——如果你给某个组授过读权限（沙箱工具、备份服务等常会这么做），那个组就能读到盐。用 `icacls "$env:USERPROFILE\.claude\vault-loader-metrics\.salt"` 可以查看实际授权。介意的话就别在 Windows 上开 metrics。
...
5. **怎么清 + 残留告知**——报表与清空：
   ...
   python3 "$VL/analyze_metrics.py" --reset-counts  # 只清 near-miss 累计计数与提示冷却；事件记录与人工标注不动
