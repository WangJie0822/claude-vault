...
argument-hint: "[--no-log] [--quick] [-f] [--catch-up [N]] [--vault <路径>] [--log-dir <路径>] [--set-default <路径>] [--show-config] [--auto --session <uuid>] [--review-drafts] [--apply-drafts] [--backfill-archive]"
...
| `-f` / `--force` | 强制模式：跳过第三步计划确认，按默认值直接执行（计划仍打印供追溯）；不自动跑 baseline / `--fix-frontmatter` / `fix_links` 等需授权操作。与 `--auto` 互斥 | `/summarize-session -f` |
| `--catch-up [N]` | 回溯模式：扫描最近 N 天（默认 7）未总结的历史会话，补充总结 | `/summarize-session --catch-up 3` |
| `--auto --session <uuid>` | 自动模式（headless cron 调用，禁用交互，CLAUDE.md/Memory 走草稿区） | `/summarize-session --auto --session abc-...` |
| `--review-drafts` | 审阅自动模式生成的 CLAUDE.md/Memory 草稿（输出 diff） | `/summarize-session --review-drafts` |
| `--apply-drafts` | 合入所有 auto-drafts/ 下草稿到目标文件 | `/summarize-session --apply-drafts` |
...
| （无参数） | 使用默认配置或当前目录 | `/summarize-session` |
...
3. 如果配置文件也没有，使用当前工作目录
4. 验证 `$VAULT` 路径存在且是目录，否则报错
...
这是引导语义：Claude 告知用户"此次会话值得用 `/summarize-session` 沉淀"；自动模式（`--auto`）的调度由 `enqueue_auto_summary.py` 硬规则（`min_messages`/`min_size_kb` 等）独立控制，与此处引导无关——不要因为此处描述触发自动总结。

### 自动模式（--auto）的入队条件说明

自动总结入队由 `enqueue_auto_summary.py` 的 `hard_rules` 决定，与上方手动建议规则无关。硬规则字段（可在 config.json 中调整，以下为默认值参考）：

| 字段 | 含义 |
|---|---|
| `min_messages` | 最小消息数阈值（不足则跳过） |
| `min_size_kb` | 最小会话文件大小（KB） |
| `min_duration_min` | 最小会话时长（分钟） |
| `require_edit_or_write` | 是否必须有 Edit/Write 工具调用 |

自动模式默认关闭（`auto.enabled: false`）；启用与调度安装见 `references/auto-mode.md`。
...
```bash
python3 ~/.claude/skills/summarize-session/scripts/obsidian_cli.py --vault "$VAULT" <op> [...]
```
...
`/summarize-session --auto --session <uuid>` 是 launchd 凌晨 cron 触发的 headless 入口（默认 02:30，仅 macOS）。行为与手动模式有以下差异：

### 1. 入口参数

- **强制带 `--session <uuid>`**，只总结指定会话；不依赖当前对话上下文
- 内部走 `scan_sessions.py --parse --session <uuid>` 解析路径

### 2. 禁用所有 AskUserQuestion

自动模式下 headless 无法交互。所有 `AskUserQuestion` 调用替换为：第三步生成更新计划后**不弹确认，直接进入第四步**；计划仍然写到 stdout，供 cron 日志追溯。

### 3. LLM 二次价值过滤

在第二步「扫描对话上下文」之后、第三步「生成更新计划」之前，**插入价值判定步骤**：

> 自我提问（LLM 内部判断，不输出选项）：
> "本会话扫描结果是否包含**对未来工作有持久价值**的内容？"
>
> 判定为 (a) 时：有持久价值的技术决策、项目进展、踩坑记录、用户偏好/规则之一 → 继续走第三步
>
> 判定为 (b) 时：仅一次性操作、纯调试、纯查询、无任何沉淀价值 → **跳过后续所有写入**，标记会话已总结（避免下次又入队），输出 `STATUS=skipped_by_llm reason=<一句话>`

判定结果**必须显式输出**到 stdout 第一行，格式 `LLM_VALUE_VERDICT=continue` 或 `LLM_VALUE_VERDICT=skip reason=<...>`。

### 4. CLAUDE.md / Memory 走草稿区

自动模式下，以下三类沉淀**不直接写入目标**，而是写入草稿区 `~/.claude/skills/summarize-session/auto-drafts/$(date +%F)/`：

| 类别 | 草稿文件命名 | 草稿 frontmatter `target_path` |
|---|---|---|
| 全局 CLAUDE.md 偏好 | `global-claude.draft.md` | `~/.claude/CLAUDE.md` |
| 项目 CLAUDE.md 规则 | `project-<basename>.draft.md` | `<git toplevel>/CLAUDE.md` |
| Memory 沉淀 | `memory-<name>.draft.md` | `~/.claude/projects/<project>/memory/<name>.md` |

草稿文件结构示例：

```markdown
---
target_path: /Users/test/.claude/CLAUDE.md
source_session: <uuid>
...
笔记 / 工作日志 / 文档归集 **不变**，直写 Vault。

### 5. 输出格式

- stdout 简短：第一行 `LLM_VALUE_VERDICT=...`，后续输出每行 `[AUTO] <内容>`
- 详细执行日志写到 `~/.claude/skills/summarize-session/auto-runs/session-<uuid>.log`（由 cron 重定向）

### 6. 标记已总结

不变。通过 `scan_sessions.py --mark <uuid>` 写 `summarized-sessions.json`，与手动模式共享清单。

## 草稿审阅模式（`--review-drafts`）

`/summarize-session --review-drafts` 用于审阅自动模式生成的待审草稿。直接调用：

```bash
python3 ~/.claude/skills/summarize-session/scripts/diff_drafts.py
```

脚本会扫描 `auto-drafts/` 下所有 `*.draft.md`，按目标类型分组，逐个输出 unified diff（目标文件 vs 追加草稿后的合并结果）。最后提示用户：**直接编辑或删除 `auto-drafts/<日期>/*.draft.md`**，再运行 `/summarize-session --apply-drafts` 一键合入。

## 草稿合入模式（`--apply-drafts`）

`/summarize-session --apply-drafts`，直接调用：

```bash
python3 ~/.claude/skills/summarize-session/scripts/apply_drafts.py
```

脚本遍历所有草稿，**Read 目标文件 → Edit 追加草稿正文**（沿用 skill 现有 Read→Edit 并发安全模式）；成功 → `rm` 草稿，失败 → 保留草稿并日志记录原因。处理完后清理空的日期目录，输出 `合入 N 项 / 失败 M 项`。

自动模式完整指南详见 `references/auto-mode.md`（跨平台安装、上线策略 dry_run 7 天、暂停/恢复、调优、故障排查）。

## 强制模式行为差异（`-f` / `--force`）

`/summarize-session -f` 在**手动流程**基础上跳过交互确认，按默认值直接推进到执行，适合无人值守 / 批量收尾。

**与 `--auto` 的区别**（二者**互斥**，同传时报错并提示改用其一）：

| 维度 | `-f`（强制手动） | `--auto`（headless cron） |
|---|---|---|
| 入口 | 当前对话上下文 | 强制 `--session <uuid>`，不依赖当前对话 |
| CLAUDE.md / Memory | **直写目标** | 走草稿区 `auto-drafts/` |
| LLM 二次价值过滤 | 无（照常沉淀） | 有（可整会话 skip） |
| 适用 | 交互会话末尾快速收尾 | 调度器 02:30 定时（跨平台，见 auto-mode.md） |
...
3. 如果是 `--review-drafts`，运行 `python3 ~/.claude/skills/summarize-session/scripts/diff_drafts.py` 并退出（详见后文「草稿审阅模式」）
4. 如果是 `--apply-drafts`，运行 `python3 ~/.claude/skills/summarize-session/scripts/apply_drafts.py` 并退出（详见后文「草稿合入模式」）
5. 如果是 `--auto --session <uuid>`，进入**自动模式**——详见后文「自动模式行为差异」一节，不执行下方正常流程（**若同时带 `-f` / `--force`，报错退出：二者互斥**）
6. 如果是 `--catch-up [N]`，进入**回溯模式**——详见 `references/catch-up.md`，不执行下方正常流程
7. 检查模式标志：
   - `--quick` → 设置 `$QUICK=true`（隐含 `$NO_LOG=true`、`$SKIP_DOC_COLLECT=true`、`$SKIP_MEMORY=true`）
   - `--no-log` → 设置 `$NO_LOG=true`
   - `-f` / `--force` → 设置 `$FORCE=true`（强制模式，跳过第三步确认，详见前文「强制模式行为差异」）；若同时出现 `--auto`，报错退出（二者互斥）

⚡ **并行执行**（同一消息中发出多个 tool calls）：
- Read `~/.claude/skills/summarize-session/config.json`
- `Bash: date "+%Y-%m-%d %H:%M"`
- `Bash: python3 ~/.claude/skills/summarize-session/scripts/scan_sessions.py --timerange "$PWD"`（从当前会话 JSONL 提取工作时段的推断初值）
- `Bash: python3 ~/.claude/skills/summarize-session/scripts/scan_sessions.py --touched-repos "$PWD"`（扫描本次会话实际修改的 git 仓库集合，解决 cwd 与实际修改仓库错位的问题）
...
python3 ~/.claude/skills/summarize-session/scripts/fix_links.py --vault "$VAULT"
python3 ~/.claude/skills/summarize-session/scripts/fix_links.py --vault "$VAULT" --apply
```
...
1. 文档归集已由 `sync_pending_docs.py` 完成；脚本通过 atomic rename 更新 pending-docs.json 字段（含 vault_path / source_*hash / vault_content_hash / source_mtime / source_size / archived_at / last_synced_at / wikilink_form / original_missing / path_invalid / denied_sensitive 等）。**LLM 不应再手动清理或修改 pending-docs.json**——有用条目保留主表，真死条目由 sync 自动 prune（删前 `.bak` 备份），状态由字段反映：
   ...
   - **一次性深度清理**（积压死条目过多时）：`reclaim_and_prune.py` 先对死条目 basename 在 Vault 唯一命中者重建 vault_path（转 active），再清理其余真死条目：
     ...
     python3 ~/.claude/skills/summarize-session/scripts/reclaim_and_prune.py --vault "$VAULT"
     # 确认后 --apply（自动 .bak 备份）
     python3 ~/.claude/skills/summarize-session/scripts/reclaim_and_prune.py --vault "$VAULT" --apply
     ```
...
2. **触发 Obsidian 重扫**（若 CLI 可用）：
   ```bash
   python3 ~/.claude/skills/summarize-session/scripts/obsidian_cli.py --vault "$VAULT" reload
   ```
   `used: fallback` 时跳过（Obsidian 未运行，无需重扫）。
3. 重建索引：
   ```bash
   python3 ~/.claude/skills/summarize-session/scripts/rebuild_index.py --vault "$VAULT" --emit=all
   ```
...
4. **解析 `health_check` 字段**——脚本输出 JSON 含：
   ...
   ```bash
   python3 ~/.claude/skills/summarize-session/scripts/rebuild_index.py --vault "$VAULT" --emit=all --fix-frontmatter
   ```
   若 `stale_indexes` 非空：建议归档：
   ```bash
   python3 ~/.claude/skills/summarize-session/scripts/rebuild_index.py --vault "$VAULT" --emit=all --archive-stale-indexes
   ```
...
6. 标记当前会话为已总结（避免 `--catch-up` 重复处理）：
   ```bash
   python3 ~/.claude/skills/summarize-session/scripts/scan_sessions.py --mark-current "$PWD"
   ```
7. **提交知识库改动（git commit）**——默认开启，`--no-commit` 跳过：
   ```bash
   python3 ~/.claude/skills/summarize-session/scripts/git_commit_vault.py --vault "$VAULT" --title "<本次会话标题>"
   ```
...
- 用户未确认前不执行任何写入操作（例外：`-f` / `--force` 强制模式与 `--auto` 自动模式由用户在调用时显式授权跳过确认）
