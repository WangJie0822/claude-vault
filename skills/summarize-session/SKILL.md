...
- **注意**：`config.json` / `auto-drafts/` / `auto-runs/` 等 **runtime 态仍在** `~/.claude/skills/summarize-session/`（稳定持久位置），下文对它们的引用保持不变。
...
3. 如果是 `--review-drafts`，运行 `python3 "$SS/diff_drafts.py"` 并退出（`$SS` 见「脚本路径」；详见后文「草稿审阅模式」）
4. 如果是 `--apply-drafts`，运行 `python3 "$SS/apply_drafts.py"` 并退出（`$SS` 见「脚本路径」；详见后文「草稿合入模式」）
