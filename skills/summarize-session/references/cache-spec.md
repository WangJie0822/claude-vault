...
```bash
# 正常运行
python3 scripts/rebuild_index.py --vault "$VAULT"

# 指定缓存路径
python3 scripts/rebuild_index.py --vault "$VAULT" --cache /path/to/cache.json

# 预览索引内容（不写入文件）
python3 scripts/rebuild_index.py --vault "$VAULT" --dry-run

# 自定义参数
python3 scripts/rebuild_index.py --vault "$VAULT" --max-log-days 14 --max-lines 300
```
